...
ruta = f"abfss://{container}@adlsxbautisv.dfs.core.windows.net/productos.csv"
...
personas_schema = StructType(fields=[StructField("id_producto", StringType(), False),
...
personas_selected_df = df_personas_final.select(col("id_producto"), 
                                                col("nombre_producto"), 
                                                col("categoria"), 
                                                col("subcategoria"))
...
personas_renamed_df = personas_selected_df.withColumnRenamed("id_producto", "id_producto") \
                                            .withColumnRenamed("nombre_producto", "nombre_producto") \
...
spark.sql(f"DROP TABLE IF EXISTS {catalogo}.{esquema}.productos")

# COMMAND ----------

personas_final_df.write.mode("overwrite").saveAsTable(f"{catalogo}.{esquema}.productos")
